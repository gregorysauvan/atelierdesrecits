...
async function callClaude(messages, maxTokens = 7000, timeoutMs = null, retries = 2, noAutoComplete = false) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      const res = await withHardTimeout(fetch("/.netlify/functions/anthropic", {
        method: "POST",
      ...
      if (!noAutoComplete && stopReason === "max_tokens" && text.trim()) {
        ...
        if (isIncomplete) {
          ...
          try {
            ...
            const compRes = await fetch("/.netlify/functions/anthropic", {
              method: "POST",
              headers: { "content-type": "application/json" },
  ...
  return (
    <div id="bk-characters" className="bi" style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 4, overflow: "hidden", marginBottom: "2rem" }}>
      <div style={{ background: C.panelBg, borderBottom: `1px solid ${C.border}`, padding: "0.75rem 1.25rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontSize: "0.65rem", letterSpacing: "0.2em", color: gold, textTransform: "uppercase" }}>Annexe — Personnages</span>
      ...
      <div style={{ padding: "1.5rem 2.5rem" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "1rem" }}>
          {localChars.filter(c => c.role !== "mentionné").map(ch => {
            ...
            return (
              <div key={ch.id} style={{ background: C.card, border: `1px solid ${isEd ? gold + "66" : C.border}`, borderRadius: 4, padding: "0.75rem 1rem", transition: "border-color 0.15s" }}>
                <div style={{ display: "flex", gap: "0.5rem", alignItems: "center", marginBottom: "0.35rem" }}>
                  {isEd ? (
                    <input value={ch.name} onChange={e => updateChar(ch.id, "name", e.target.value)}
                      style={{ flex: 1, background: C.panelBg, border: `1px solid ${gold}55`, borderRadius: 3, color: gold, fontFamily: typo.titleFont, fontWeight: "bold", fontSize: "0.9rem", padding: "0.1rem 0.4rem" }} />
                  ) : (
                    <span style={{ fontFamily: typo.titleFont, fontWeight: "bold", color: gold, fontSize: "0.9rem", flex: 1 }}>{ch.name}</span>
                  )}
                  {isEd ? (
                    ...
                    <span style={{ fontSize: "0.6rem", color: C.muted, background: `${gold}18`, borderRadius: 3, padding: "0.05rem 0.3rem" }}>{ch.role}</span>
                  )}
                  <button onClick={() => setEditingCharId(isEd ? null : ch.id)}
                    style={{ background: "transparent", border: "none", color: isEd ? gold : C.muted, cursor: "pointer", fontSize: "0.7rem", padding: "0 0.2rem", opacity: isEd ? 1 : 0.5 }}>
                    {isEd ? "✓" : "✎"}
                  </button>
                </div>
                {isEd ? (
